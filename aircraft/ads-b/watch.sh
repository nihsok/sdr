...
echo 'flag,alt,lon,lat,gs_x,gs_y,u,v,vt_x,vt_y,tas,t,mach,cas,ias,roll,p,dist,flight,squawk,category,version,hex,time' | cat - /tmp/dump1090-fa/history_*.csv | sort -g | curl http://localhost/~$LOGNAME/web/upload.php -X POST -F 'file=data.csv' -F data=@-
...
done) | curl http://localhost/~$LOGNAME/web/upload.php -X POST -F 'file=dz.csv' -F data=@-
