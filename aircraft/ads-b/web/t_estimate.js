d3.csv("./data.csv").then(function(data){
  ...
  pressure.append("g")
    .attr("transform", "translate(0," + height + ")")
      .call(d3
        .axisBottom(x.domain([150,850]))
        .tickValues(d3.range(150,851,50))
      ...
      .attr("x", width / 2 )
      .attr("y", 40)
      .text("Pressure from IAS [hPa]")
  pressure.append("g")
    .call(d3
      .axisTop(x.domain([150,850]))
      .tickValues(d3.range(150,850,50))
      .tickFormat(''))
  //y axis
  pressure.append("g")
    .call(d3
      .axisLeft(y.domain([150,850]))
      .tickValues(d3.range(150,851,50))
      .tickFormat(val => val % 100 == 0 ? val : ''))
    .style("font-size",20)
    .append("text")
      ...
      .attr("y", -50)
      .attr("transform","rotate(-90)")
      .text('Pressure from barometer [hPa]')
  pressure.append("g")
    .attr('transform',"translate(" + width + ",0)")
    .call(d3
      .axisRight(y.domain([150,850]))
      .tickValues(d3.range(150,850,50))
      .tickFormat(''))
  ...
  pressure.selectAll("dot")
    .data(data.filter(d => d.p_baro && d.p > 0))
    .enter()
    .append("circle")
      .attr("cx", d => x(d.p/100))
      .attr("cy", d => y(d.p_baro/100))
      .attr("r", 3)
      .style("fill", d => '#'+d.hex)
      .style("opacity", 0.5)
    .on("mouseover",function(event,d){
      ...
      tooltip
        .style("visibility","visible")
        .html('P(estimated): '+Math.round(d.p)/100+'hPa'+'<br>P(barometer): '+Math.round(d.p_baro)/100+'hPa')
    })
    .on("mousemove",function(event){
      tooltip
        .style("left", event.pageX + 15 + "px")
