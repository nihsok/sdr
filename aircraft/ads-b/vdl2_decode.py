			...
			if ( result := re.findall(r"[ \d]{2}\d00[NS]\d{5}[EW]\d{10}[- ][- \d]\d{4} [ \d]\d",data['acars']['msg_text']) ): #pattern 0
				for code in result:
					...
					print ( int(code[0:5]) * 0.3048, \
						...
						-wind_s * math.sin(wind_d), -wind_s * math.cos(wind_d), int(code[22:25]) + 273.15, '0', code, sep=',')
				...
				for code in result:
					...
					print ( int(code[17:22]) * 0.3048,\
						...
						-wind_s * math.sin(wind_d), -wind_s * math.cos(wind_d), int(code[22:25]) + 273.15, '1', code, sep=',')
				...
				for code in result:
					...
					print ( int(code[17:22]) * 0.3048,\
						...
						( -1 if code[22]=='M' else 1 ) * int(code[23:25]) + 273.15, '1.5',code, sep=',')
