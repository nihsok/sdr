...
with open(sys.argv[1]) as f:
	for line in f:
		...
		if ('acars' in data and 'msg_text' in data['acars']):
			if ( result := re.findall(r"[ \d]{2}\d00[NS]\d{5}[EW]\d{6}[-\d]{4}[- ][- \d]\d{4} [ \d]\d",data['acars']['msg_text']) ): #pattern 0
				for code in result:
					wind_d = int(code[25:28]) * math.pi/180 #deg->radian
					wind_s = int(code[28:31]) * 0.51444 #kt->m/s
					print('0', ('' if code[18:22]=='----' else code[18:20]+':'+code[20:22]), int(code[0:5]) * 0.3048, \
						( -1 if code[ 5]=='S' else 1 ) * int(code[6:8]) + int(code[8:11]) / 600, \
						( -1 if code[11]=='W' else 1 ) * int(code[12:15]) + int(code[15:18]) / 600, \
				...
				for code in result:
					...
					print('1', code[13:15]+':'+code[15:17], int(code[17:22]) * 0.3048,\
						( -1 if code[0]=='S' else 1 ) * int(code[1:3]) + int(code[3:6]) / 600, \
						( -1 if code[6]=='W' else 1 ) * int(code[7:10]) + int(code[10:13]) / 600, \
						-wind_s * math.sin(wind_d), -wind_s * math.cos(wind_d), \
				...
				for code in result:
					wind_d = int(code[27:30]) * math.pi/180
					wind_s = int(code[30:33]) * 0.51444
					print('2.1', code[13:15]+':'+code[15:17]+':'+code[17:19], int(code[19:24]) * 0.3048,\
						( -1 if code[0]=='S' else 1 ) * int(code[1:3]) + int(code[3:6]) / 600, \
						( -1 if code[6]=='W' else 1 ) * int(code[7:10]) + int(code[10:13]) / 600, \
						-wind_s * math.sin(wind_d), -wind_s * math.cos(wind_d), \
						int(code[24:27]) + 273.15, code, sep=',')
